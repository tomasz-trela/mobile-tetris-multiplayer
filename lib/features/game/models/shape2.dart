...
class Shape1 {
  ...
  Shape1(this.coordinates, this.color, this.center);
  ...
  Shape1 copyWith({List<Coordinates>? coordinates, Color? color, Coordinates? center}) {
    List<Coordinates> newCoordinates = [];
    for (var element in coordinates ?? this.coordinates) {
      newCoordinates.add(element.copyWith());
    }
    return Shape1(coordinates ?? newCoordinates, color ?? this.color, center ?? this.center.copyWith());
  }
